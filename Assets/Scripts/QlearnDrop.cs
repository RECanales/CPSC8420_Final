...
public class QlearnDrop : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (scene_obj.GetComponent<CollisionDetector>().reached_goal && !check)
        {
            check = true;
        }
        if (check && !handControl.positioned_over_target)
        {
            ...
            else
            {
                ...
                if (!done)
                {
                    ...
                    if (!written)
                    {
                        ...
                        using (var writer = new StreamWriter("Dropping_Policy.csv"))
                        {
